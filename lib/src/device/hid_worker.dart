...
///
/// Creates its own [HidapiHidBackend], opens a read handle, and runs a
...
void hidWorkerEntryPoint(WorkerStartup startup) {
  ...
  // command port listener drain pending shutdown before the next read.
  // The blocking read IS the yield: the event loop runs while the
  // synchronous FFI call waits for USB data.
  //
  ...
  Future<void>(() async {
    while (running) {
      ...
      // Yield to drain pending shutdown command.
